...
class GRMHomePage(Document):
	def validate(self):
		self.make_files_public()

	def make_files_public(self):
		"""Ensure all attached files are stored in the public directory."""
		for fieldname in ("hero_img", "about_img", "workflow_img", "testimonials_img"):
			_set_file_public(self.get(fieldname))

		for row in (self.features or []):
			_set_file_public(row.feature_icon)

		for row in (self.testimonials or []):
			_set_file_public(row.test_img)
